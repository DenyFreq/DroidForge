...
//                Disposer.register(this, myAppResources);
    ...
    private Map<ExternalAndroidLibrary, AarResourceRepository> computeLibraryResourceMap() {
        List<DroidForgeAndroidLibrary> codeAssistLibraries =
                ((AndroidModuleImpl) myFacet).getDroidForgeLibraries().stream()
                        .filter(it -> it instanceof DroidForgeAndroidLibrary)
                        .map(it -> (DroidForgeAndroidLibrary) it)
        ...
        for (DroidForgeAndroidLibrary library : codeAssistLibraries) {
